...
문자 바꾸기
    ...
    trnaslate()로 적용한다.
    아래테이블은 a는 1, e 는 2로 변경된다.
    '''
print(f"{'maketrans()/trnaslate':-^30}")
str1 = "apple"  
table = str1.maketrans('aeiou','12345')
print(str1.translate(table))


#한아언이 가 'xyz' 로 각각 변경된다.
str2 = '한글은 아름다운 언어'
...
'''
열 위치 찾기
    find(왼쪽부터), rfind(오른쪽부터) 문자열을 찾은 후
    인덱스를 반환
'''
...
print(f"{'strip()':-^30}")
str = 'python312좋아'
print(str.isalnum()) # 트루
str = 'python3.12좋아^^'
print(str.isalnum()) # 펄스

'''
시나리오] 입력한 문자열에 영문대문자, 소문자, 숫자만 포함되어
          있다면 True, 나머지 문자가 하나라도 포함되면 False를 반환하는 프로그램을 작성하시오.
'''

print(f"{'시나리오':-^30}")
s = input('문자열을 입력하세요:')
result = True
#입력받은 문자열을 통해 길이만큼 반복한다.
for char in s:
    # 문자를 하나씩 검사한다.
    if not (char.isupper() or char.islower() or char.isdigit()):
        #대문자,소문자,숫자가 아닌 경우 False 를 할당
        result = False
